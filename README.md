...
5 - Mean of basal and % differences betwenn before and after molecules effect for each replicates and concentrations are extracted with a MATLAB script (Contractility_analysis_Well_96tables_gaussian_filter_amplitude).

# Isotonic force transducer method analysis (emkaBATH4) 
This MATLAB script is applied for each organ bath. Raw force in grams is extracted from text file that are generated from the iox software included with the emka machine and are copy-paste in the first column of a matrix called 'data'. It traces Forces (g) against Time (min) for all experiment and each concentration (0, 0.01, 0.1, 1 and 10 µM)(Contractility_analysis_forceTransducer_gaussian_filter).
